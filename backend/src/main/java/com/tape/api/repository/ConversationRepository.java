...
public interface ConversationRepository extends JpaRepository<Conversation, String> {
    ...
    /**
     * Deletes all conversations the user participates in. Run only after the
     * conversations' messages have been deleted (FK constraint).
     */
    @Modifying
    @Query("DELETE FROM Conversation c WHERE c.participant1.id = :userId OR c.participant2.id = :userId")
